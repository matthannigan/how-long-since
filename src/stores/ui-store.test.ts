...
describe('useUIStore', () => {
  ...
  it('shows an undo snackbar carrying the prior Date', () => {
    const previous = new Date('2026-06-01T12:00:00Z');
    useUIStore.getState().showUndo('task-1', previous);
    expect(useUIStore.getState().undoSnackbar).toEqual({ taskId: 'task-1', previous });
  });

  it('round-trips a null previous (first completion) without coercing it', () => {
    useUIStore.getState().showUndo('task-2', null);
    expect(useUIStore.getState().undoSnackbar).toEqual({ taskId: 'task-2', previous: null });
  });
